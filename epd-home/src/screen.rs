...
fn save_to_bmp<T: Write>(pixmap: Pixmap, output_file: &mut T) {
    ...
    let data = (0..pixmap.height())
        ...
        .collect_vec();
    let bmp = Bmp::new(data).unwrap();
    bmp.write(output_file).unwrap();
}
...
async fn render_svg<T: Write>(svg_data: Vec<u8>, bmp_file: &mut T) {
...
impl Screen {
    pub fn new(latitude: f64, longitude: f64, timezone: &str, stop_code: &str) -> Result<Self> {
    ...
    async fn gather_weather(&self) -> Result<(WeatherData, Vec<WeatherData>)> {
        ...
        let current = WeatherData {
            ...
            temp: Some(weather.current.temperature.round() as i64),
        };
        ...
        let hourly_data = weather
            ...
            .map(|data| {
                let d = WeatherData {
                    ...
                    temp: Some(data.temperature.round() as i64),
                };
                Ok::<_, Error>(d)
            })
    ...
    async fn gather_arrivals(&self) -> Result<Vec<ArrivalData>> {
        let arrivals = get_stop_arrivals(&self.stop_code).await?;
        let now = Utc::now().with_timezone(&self.timezone);
    ...
    pub async fn render<T: Write>(&self, to_bmp: &mut T) -> Result<()> {
        ...
        render_svg(svg_data, to_bmp).await;
